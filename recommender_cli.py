...
def recommend_books(topics, count, min_rating, genres):
    ...
    return results or "No books found matching your filters."

# بدء التشغيل من الطرفية
if __name__ == "__main__":
    books = load_books()
    model = SentenceTransformer('all-MiniLM-L6-v2')
    index = load_or_create_index(books, model)

    print("Welcome to the Book Recommendation CLI")
    topics = input("Enter topics (comma-separated): ")
    try:
        count = int(input("Number of books to recommend: "))
    except:
        count = 5
    try:
        ...
        min_rating = 0
    genres = input("Enter genres (optional, comma-separated): ")

    output = recommend_books(topics, count, min_rating, genres)
    print("\nRecommendations:\n")
    print(output)
